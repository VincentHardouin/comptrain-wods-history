  ...
  <div class="min-h-screen bg-base-200">
    ...
    <section class="px-4 pb-20 max-w-7xl mx-auto space-y-6">
      ...
      <div v-else class="text-center py-20 text-base-content/40">
        ...
        <button @click="clearFilters" class="btn btn-primary btn-sm mt-4">Reset filters</button>
      </div>
    </section>
  </div>
</template>
...
function parseWodType(content: string): string[] {
  ...
  return WOD_TYPE_PATTERNS.filter(({ patterns }) => patterns.some((p) => lower.includes(p.toLowerCase()))).map(
    ({ label }) => label,
  );
}
